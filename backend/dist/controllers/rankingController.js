...
const getRanking = async (req, res) => {
    try {
        const { jobDescription = '', requiredSkills = [], preferredSkills = [], minExperience = 0, limit = 20, gender, cities = [], jobLat, jobLon, maxDistanceKm } = req.body || {};
       ...
       LEFT JOIN users ON users.id = cvs.user_id`);
        ...
        const haversine = (lat1, lon1, lat2, lon2) => {
            if ([lat1, lon1, lat2, lon2].some((x) => x === null || x === undefined))
                return undefined;
            const R = 6371; // km
        ...
        const filtered = scored
            ...
            .filter((c) => {
            if (maxDistanceKm == null || jobLat == null || jobLon == null)
                return true;
